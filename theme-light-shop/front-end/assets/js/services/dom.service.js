 ...
 */
export function setOverlayVisible($element, visible, isBottomSheet = false) {
  const isMobile = window.innerWidth < 768;
  
  if (visible) {
    $element.removeClass("opacity-0 pointer-events-none");
    if (isBottomSheet && isMobile) {
      $element.find("#checkout-panel").addClass("translate-y-0").removeClass("translate-y-full");
      document.body.classList.add("checkout-open");
    }
    return;
  }
  $element.addClass("opacity-0 pointer-events-none");
  if (isBottomSheet && isMobile) {
    $element.find("#checkout-panel").removeClass("translate-y-0").addClass("translate-y-full");
